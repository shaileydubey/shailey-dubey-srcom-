// ======================== Settings Component ========================
// Fully connected to DB — create, edit, delete, toggle agent status
// All changes persist to MySQL via FastAPI backend
// ====================================================================
...
function AgentModal({ agent, onClose, onSave }) {
  // Pre-populate with existing agent data (snake_case from DB)
...
export default function Settings() {
  ...
  // ---------------------------------------------------------------
  // SEARCH — snake_case to match DB response
  // ---------------------------------------------------------------
  ...
  // ---------------------------------------------------------------
  // CREATE or UPDATE — persists to DB
  // ---------------------------------------------------------------
  const handleSave = async (form, isEdit) => {
    try {
      ...
      if (isEdit) {
        setAgents(prev => prev.map(a => a.id === form.id ? { ...a, ...payload } : a));
  ...
  // ---------------------------------------------------------------
  // TOGGLE individual agent status — persists to DB
  // ---------------------------------------------------------------
  ...
  // ---------------------------------------------------------------
  // DERIVED COUNTS from DB is_active field
  // ---------------------------------------------------------------
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020617", color: "#e2e8f0", fontFamily: "'Inter', sans-serif", padding: "32px 40px" }}>
      ...
      {modal && (
        <AgentModal
          agent={modal.type === "edit" ? modal.agent : null}
